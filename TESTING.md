...
- **New Vercel projects created via API/manual file upload, later Git-linked: pushes do not reliably auto-deploy.** Confirmed Sep 8, 2026: after connecting a GitHub repo and setting Production branch tracking, only the very first push after that connection auto-deployed — every push since required a manual "Redeploy" click in the Vercel dashboard on the specific new commit. Do not assume a pushed fix is live; always confirm the deployed commit SHA (dashboard or live tool re-fetch) before retesting, and expect to ask for a manual redeploy click as the default, not the exception.
- **Multiple near-identically-named connectors active at once confuses host tool selection**, independent of any one connector's description quality — confirmed Sep 8, 2026 when ChatGPT struggled to pick between `CarClever`, `CarClever V2`, `CarClever V2 Test`, `CarClever V3 Test` (three sharing the same long V1-style description). Fix was renaming the connector under test to something visually distinct (not sharing the "CarClever" prefix) rather than disconnecting the others — much lower overhead.
- **Claude's connector client does not surface a tool's `structuredContent` to the user or back to the model, regardless of whether the tool declares an `outputSchema`** (tested explicitly — adding one did not fix it). ChatGPT's client does surface it reliably. Confirmed platform-level limitation, not a probe/schema bug. Practical effect: any Claude-side evidence in a host-comparison test has to rely on Claude self-reporting what it sent, not raw verifiable JSON — treat as directionally useful, not byte-exact confirmed.

### Sep 8, 2026 — `research/v2-schema-probe` branch — Claude + ChatGPT — V2 shared-contract discovery run

- Tester: Claude (Engineering lane), André running host-side prompts on both Claude and ChatGPT.
- Scope: schema-only probe (no Auto.dev/NHTSA/search — echoes input + validation stats only), isolated Vercel project `carclever-v2-schema-probe`, connector renamed to "Test V2" after naming collision with the live `CarClever *` connectors caused ChatGPT tool-selection confusion (unrelated to schema content — see new recurring-issue note below).
- Purpose: test whether the candidate short public description (replacing V1's long operating-manual style) reliably produces V1-equivalent host behavior, ahead of any real V2 implementation.
- **3 real regressions found and fixed** (full detail: `getcarwise-docs/FINAL_FINDINGS_20260908.md`, `CHANGE_LOG.md` on the research branch):
  1. Practical-need → model-list resolution: initially failed on ChatGPT entirely (Claude did it correctly from the start). Root cause chain: missing field-level descriptions → abstract principle insufficient → needed concrete worked examples → examples alone still unreliable → fixed by switching to imperative voice matching V1's actual working pattern ("resolve it... every time", "using your own knowledge, before calling this tool"). Confirmed 2/2 clean on ChatGPT (raw JSON), 1/1 clean on Claude (self-report only, see known-issue below).
  2. Manufacturer names leaking into cross-brand model lists (e.g. "Honda CR-V" instead of "CR-V") — would have silently zeroed real Auto.dev results. Fixed with an explicit cross-brand example in the field description.
  3. `priorityAxis` missing V1's "budget ≠ cheapest" disambiguation — restored from V1, confirmed correct on both values.
- Ambiguous-city clarification, electrification required/preferred split, named hybrid/PHEV variants, and fragile direct fields (color/cylinders/vehicleType) all confirmed clean under the short description with no fix needed.
- **Not yet done:** real evidence-based cap for `model` field (still using a generous research ceiling); provisional 50-char text-field caps untested; independent second-tester pass recommended given how much run-to-run host variance was observed even after fixes.
