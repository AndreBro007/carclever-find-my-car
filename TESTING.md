...
- First pass (Haiku 4.5, `TEMP V3.2 Branch Test`): "Check VIN ... for recalls and any red flags" returned `Error: Failed to fetch` with a fabricated narrative response ("results are displaying in the widget above" — no widget shown). Root-caused via Vercel Runtime Logs: all 12 requests in the window were `POST /mcp → 200` with zero server errors, and none showed the outgoing Auto.dev/NHTSA calls `check_vehicle`'s VIN path always makes — meaning the tool was never actually invoked with the VIN. Confirmed by re-running the identical prompt on Sonnet 5: passed immediately, with the real external calls visible in the logs (see below). Conclusion: Haiku 4.5 failed to execute the tool call correctly; not a defect in this session's `check_vehicle` code.
- Full combined scenario pass (Sonnet 5, new temp connector `TEMP V3 Combined Test`, same branch preview):
  1. `find_matching_vehicle` — "find a used SUV under $30,000 near 90210" → real card, 8 strong matches, USED badges, split Check avail./View similar CTAs — V2 search behavior unaffected.
  2. `check_vehicle` follow-up — "Check the first result (2025 Ford Escape Plug-In Hybrid) for recalls and red flags" → **the exact same listing card preserved** (same VIN, dealer, price), now with a RISK badge, Buyer Check "Promising", 9 real NHTSA recalls (3 shown with real campaign numbers). This is the core "is this a good buy?" card-loss regression (`SYS-20260906-001`/`SYS-20260906-002`) V3 set out to fix — confirmed fixed live.
  3. `resolve_dealer_url` — "Get me the link for the second result (Volvo XC40)" → correct tool routed to (required approval, since it's a read-only tool set to "Needs approval" on this temp connector), real Edmunds affiliate link returned. Untouched by this session's changes, confirmed still correct.
  4. `check_vehicle` standalone, VIN with no listing (fabricated VIN, real checksum) → correctly stayed text-only (no card — V3.3 not built yet), honestly reported no inventory match, still surfaced the NHTSA VIN decode. No invented listing.
  5. `check_vehicle` standalone, make/model/year only (no VIN) — "Any recalls for a 2023 Honda CR-V? No VIN available" → correctly stayed text-only, real recall data, correctly disclosed no Buyer Check is possible without a VIN.
- Verified via Vercel Runtime Logs (not just chat output): the follow-up VIN-check request from Sonnet 5 shows `GET api.auto.dev/listings/...`, `GET vpic.nhtsa.dot.gov/api/vehicle...`, and `GET api.nhtsa.gov/recalls/...` all firing and returning 200, plus a `GET /api/img-proxy` confirming the photo pipeline executed. One unrelated transient `502` on a single third-party photo fetch (`retail.photos.vin/...`) during the search scenario — matches the already-documented "silently skip a broken photo asset" behavior (`SYS-20260812-014`), not a regression.
- Full local regression suite re-run after live testing, from the same clean branch state: `tsc --noEmit` clean, 66/66 tests pass, `next build` clean.
- Both temporary connectors removed after testing, per standing convention.
- **Not yet tested this pass:** ChatGPT (design doc's test matrix calls for both hosts; not done in this session). V3.3 (standalone VIN card) not yet built, so its scenarios above intentionally stayed on the pre-V3.3 text-only path.
