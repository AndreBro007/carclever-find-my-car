...
Notes for decomposing the request: (1) Hybrid and plug-in hybrid vehicles are often mistagged in the source data — if the user names a specific model (e.g. "Sportage" or "RAV4"), prefer setting model to include both the base and hybrid variant name (e.g. "RAV4,RAV4 Hybrid" or "RAV4,RAV4 Prime") rather than relying on a fuel filter alone; if no model is named, fuel-based hybrid/PHEV filtering has known partial coverage — mention that to the user. (2) Map descriptive intent to the dedicated fields (bodyType, seatsMinPreference, goals) rather than into the make/model text. (3) Set priceFlexibility to "flexible" only if the user signals approximation ("around," "roughly," "about") — otherwise price stays a hard ceiling, never silently loosened. (4) SIZE AND STYLE QUALIFIERS ("large SUV," "compact SUV," "sports sedan," "off-road capable," etc.): the underlying data has no size-tier field — bodyType alone returns every SUV from subcompact to full-size undifferentiated. Do NOT rely on filtering or reviewing results after the search to catch this — by then the right vehicles may never have been fetched at all, since results aren't sorted by size. Instead, resolve the qualifier BEFORE searching: populate the model field with a comma-separated list of specific real model names you know fit the requested class (e.g. for "large SUV": "Suburban,Tahoe,Expedition,Sequoia,Wagoneer,Grand Wagoneer,Yukon XL,Yukon,Armada,Land Cruiser"), the same way you already would for a named plug-in hybrid model. This is the single most important accuracy lever for vague size/style requests — get the model list right and the search does the rest. If you genuinely don't have confident knowledge of which models fit, say so rather than guess, and fall back to bodyType alone with a caveat to the user.`;
