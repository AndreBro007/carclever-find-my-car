...
const FindMatchingVehicleInput = z.object({
  ...
  vehicleType: z.string().optional().describe("Finer body classification than bodyType, e.g. Crossover, SUV, Sedan, Wagon, Minivan, Performance/Sports, Hybrid, Hatchback, Coupe, Luxury, Electric — but these values are mutually exclusive with each other in the underlying data, including Sedan vs Luxury: a genuine luxury sedan (e.g. Mercedes E-Class, Audi A6, BMW 5-Series) is tagged Luxury there, not Sedan, so vehicleType: \"Sedan\" combined with a luxury model silently returns zero even though the car exists and is a sedan. For a plain body-style request ('sedan', 'SUV', 'hatchback'), use bodyType instead — it doesn't have this conflict and is the safer default (confirmed: bodyType 'Sedan' + a luxury model returns real results). Reach for vehicleType only for a genuinely finer distinction bodyType can't express, and never combine vehicleType: \"Sedan\"/\"Coupe\"/\"Wagon\"/\"Hatchback\" with a known-luxury make or model."),
