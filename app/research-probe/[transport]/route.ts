...
const probeShape = {
  ...
  priorityAxis: z.enum(["best_for_budget", "cheapest", "lowest_mileage", "newest", "lower_risk"]).describe("Ranking objective: best_for_budget, cheapest, lowest_mileage, newest, or lower_risk. Lower risk ranks available purchase-risk evidence and is not a guarantee.").optional(),
  ...
  model: z.string().max(PROBE_MODEL_MAX).describe("One or more real vehicle model names, without the manufacturer (for example, 'E-Class' rather than 'Mercedes-Benz E-Class'). Comma-separated for multiple models. When a practical need implies a vehicle class (for example, a large family SUV, or a reliable commuter car), likely matching real model names are resolved and included here (for example, CR-V, RAV4, Highlander for a family SUV) alongside the related need stated in vehicleNeeds.").optional(),
  ...
  vehicleNeeds: z.array(z.string().max(60)).max(5).describe("Short, capped list of listing-relevant practical needs (for example, a large family SUV or a commuter vehicle); not a transcript or broad profile. When a listed need implies a vehicle class, likely matching real model names are resolved and included in `model` alongside it (for example, a large family SUV need pairs with a model list like CR-V, RAV4, Highlander).").optional(),
