...
const probeShape = {
  ...
  vehicleNeeds: z.array(z.string().max(60)).max(5).describe("Short, capped list of listing-relevant practical needs (for example, a large family SUV or a commuter vehicle); not a transcript or broad profile. When a listed need implies specific vehicle types, likely matching models are included in `model` alongside it.").optional(),
