...
export const FindMatchingVehicleInput = z.object({
  ...
  vehicleNeeds: z.array(z.string()).optional().describe("Freeform buyer needs like 'family', 'reliability', 'commuting'. Ranking/context input only, not a hard filter — this tool has no reliability or ownership-cost data to verify these claims against. Replaces the retired `goals` field — no dual-accept — sending it produces a schema error directing the caller to vehicleNeeds."),
  electrificationTypes: z.array(z.enum(["hybrid", "plug_in_hybrid", "electric"])).optional().describe("Which electrified powertrain type(s) satisfy the request. 'hybrid' implicitly includes mild hybrids at match time — mild_hybrid is an internal NHTSA classification only and is never a selectable value here. 'plug_in_hybrid' is distinct and never implied by 'hybrid'. Combine with electrificationRequirement to say whether this is a hard requirement or a ranking preference. For a broad request without a named model, pair this field with resolved model/variant names in model; do not use electrification fields alone for a generic body-style search."),
  electrificationRequirement: z.enum(["required", "preferred"]).optional().describe("'required' excludes any vehicle NHTSA cannot confirm as one of electrificationTypes (unconfirmed/ambiguous vehicles are dropped, never assumed to satisfy the request). 'preferred' only affects ranking — no vehicle is excluded for unconfirmed electrification — and currently only nudges ordering under the default (best_for_budget) and lower_risk priorityAxis values; it has no effect on cheapest, lowest_mileage, or newest, which always follow their exact stated sort regardless of electrification."),
